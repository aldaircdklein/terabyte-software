...
export const ListaVendas = async (dados) => {
    ...
    if(dados.tipo === 'true'){
        if(dados.dateStart && dados.dateEnd){
            result = await api.get(ServeRoutes(`startDate=${dados.dateStart}&endDate=${dados.dateEnd}${'&paid='+dados.paid}&searchMode=${dados.searchMode}`).listServiceOrderUnpaid);
        }else{
            let data = new Date();
            data.setDate(data.getDate() + 2);
            result = await api.get(ServeRoutes(`startDate=2019-01-01&endDate=${format(data, 'yyyy-MM-dd')}${'&paid='+dados.paid}&searchMode=${dados.searchMode}`).listServiceOrderUnpaid);
        }
    }else{
        if(dados.dateStart && dados.dateEnd){
